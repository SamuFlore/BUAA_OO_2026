...
public class ElevatorCtrl {
    public static Action getAction(int curFloor,
                                   int direction,
                                   int curWeight,
                                   LinkedList<PersonRequest> people,
        ...
        synchronized (reqQueue) {
            LinkedList<PersonRequest> newPeople = reqQueue.getRequests();
            ...
            if (!people.isEmpty()) {
                ...
                return (targetFloor > curFloor) ? Action.UP : Action.DOWN;
            }
            if (!newPeople.isEmpty()) {
                ...
                if (hasForward) {
                    return (direction == 1) ? Action.UP : Action.DOWN;
                }
                else { // 同方向没有，就在反方向
                    return (direction == 1) ? Action.DOWN : Action.UP;
                }
            }
        }
        // 彻底没活了
    ...
    // 电梯和乘客要去的地方是否同向
    public static boolean isSameDirect(PersonRequest person, int direction, int curFloor) {
    ...
    public static boolean hasRequestForward(int curFloor, int direction,
                                            LinkedList<PersonRequest> newPeople) {
        for (PersonRequest person : newPeople) {
