...
public class MyJvm {//包含题目[3]
    ...
    /*@ public normal_behavior
      @ assignable heap.elements, heap.elements[*], heap.size, heap.capacity, MyObject.totalId;
      ...
      @              (\exists int j;
      @                   1 <= j && j <= heap.size;
      @                   heap.elements[j].equals(\old(heap.elements[i]))));
      @ ensures heap.capacity >= \old(heap.capacity);
      @ ensures MyObject.totalId == \old(MyObject.totalId) + count;
      @ also
      @ requires \old(heap.size) >= DEFAULT_CAPACITY - count;
      @ ensures heap.size == (\sum int k;
      @                           1 <= k && k <= \old(heap.size) &&
      @                           \old(heap.elements[k].isReferenced());
      @                           1) + count;
      @ ensures (\forall int i;
      @              1 <= i && i <= \old(heap.size);
