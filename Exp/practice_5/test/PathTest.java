...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        if (hasLoop && oldPath.isValid()) {
            ...
            List<Integer> resultNodes = result.getNodes();
            // \result.nodes[0] == \result.nodes[\result.nodes.length - 1]
            assertEquals(resultNodes.get(0), resultNodes.get(resultNodes.size() - 1));
            // (\exists int i; 0 <= i && i < nodes.length-1;
            // (\forall int j; 0 <= j && j < \result.nodes.length;
            // (i + j) < nodes.length && \result.nodes[j] == nodes[i + j]))
            ...
            for (int i = 0; i < oldNodes.size() - 1; i++) {
                ...
                for (int j = 0; j < resultNodes.size(); j++) {
                    if (i + j >= oldNodes.size() || !oldNodes.get(i + j).equals(resultNodes.get(j))) {
                        match = false;
                        break;
                    }
                }
        ...
        for (int i = 0; i < oldNodes.size() && allDistinct; i++) {
            for (int j = i + 1; j < oldNodes.size(); j++) {
                if (oldNodes.get(i).equals(oldNodes.get(j))) {
                    allDistinct = false;
                    break;
        ...
        if (allDistinct || !oldPath.isValid()) {
            /*
            public normal_behavior
            assignable \nothing;
            ensures \result == null;
