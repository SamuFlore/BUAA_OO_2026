...
public class Expr {
    ...
    public boolean isFactor() {
        ...
        // 如果是常数
        if (term.getExponent().equals(BigInteger.ZERO) && term.getExpContent().isEmpty()) {
            return true;
        }
        // 如果是 1 * x ^ n * exp(0) 或 1 * x ^ 0 * exp(expContent)
        if (coeff.equals(BigInteger.ONE)) {
            return ((term.getExponent().signum() > 0
                        && term.getExpContent().isEmpty()) ||
                    (term.getExponent().equals(BigInteger.ZERO)
                            && !term.getExpContent().isEmpty()));
        }
        return false;
    }
    ...
    public Expr exprPow(int exponent) {
        ...
        if (exponent == 0) {
            // 1*x^0*exp(0)
            res.addTerm(new Term(BigInteger.ZERO, new Expr()), BigInteger.ONE);
            return res;
        }
        res = this;
    ...
    // coeff * x ^ 0 * exp(0), public method
    public static Expr createConst(BigInteger coeff) {
        Expr res = new Expr();
        res.addTerm(new Term(BigInteger.ZERO, new Expr()), coeff);
        return res;
    }

    // 1 * x ^ exponent * exp(0)
    public static Expr createVar(BigInteger exponent) {
        Expr res = new Expr();
        res.addTerm(new Term(exponent, new Expr()), BigInteger.ONE);
        return res;
    }

    // 1 * x ^ 0 * exp(expContent)
    public static Expr createExp(Expr expContent) {
        Expr res = new Expr();
        res.addTerm(new Term(BigInteger.ZERO, expContent), BigInteger.ONE);
        return res;
    }
