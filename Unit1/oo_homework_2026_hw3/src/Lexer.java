public class Lexer {
    ...
    private void parseIdf() {
        if (input.startsWith("f{n-1}", pos)) {
            curType = TokenType.FUNC;
            curToken = "f{n-1}";
            pos += 6;
            return;
        }
        if (input.startsWith("f{n-2}", pos)) {
            curType = TokenType.FUNC;
            curToken = "f{n-2}";
            pos += 6;
            return;
        ...
        if (idf.equals("exp")) { curType = TokenType.EXP; }
        else if (idf.equals("x")) { curType = TokenType.VAR; }
        else if (idf.equals("y")) { curType = TokenType.VARY; }
        else if (idf.equals("f")) { curType = TokenType.FUNC; }
        else if (idf.equals("dx")) { curType = TokenType.DX; }
        else if (idf.equals("dy")) { curType = TokenType.DY; }
        else if (idf.equals("grad"))  { curType = TokenType.GRAD; }
    }

    // 解析符号
    private void parseSymbol(char c) { // why must default ???
        curToken = String.valueOf(c); // char -> string
        switch (c) {
            case '+': curType = TokenType.ADD;
                break;
            case '-': curType = TokenType.SUB;
                break;
            case '*': curType = TokenType.MUL;
                break;
            case '^': curType = TokenType.POW;
                break;
            case '(': curType = TokenType.LBRA;
                break;
            case ')': curType = TokenType.RBRA;
                break;
            case '[': curType = TokenType.LSBRA;
                break;
            case ']': curType = TokenType.RSBRA;
                break;
            case '{': curType = TokenType.LCBRA;
                break;
            case '}': curType = TokenType.RCBRA;
                break;
            case '?': curType = TokenType.QUEST;
                break;
            case ':': curType = TokenType.COLON;
                break;
            case '=': {
