...
public class Network implements NetworkInterface {
    ...
    public void watchVideo(int userId, int videoId)
        ...
        user.addTypeCnt(video.getType());
        up.addInfluence(video.getType(), 2);
    ...
    public void likeVideo(int userId, int videoId)
        ...
        if (user.hasLikedVideo(video)) {
            ...
            // add influence
            User up = getUser(video.getUploaderId());
            up.addInfluence(video.getType(), -3);
    ...
    public void coinVideo(int userId, int videoId, int amount)
        ...
        // add influence
        up.addInfluence(video.getType(), amount * 5);
    ...
    public void forwardVideo(int userId, int videoId, int followerId)
        ...
        // add influence
        up.addInfluence(video.getType(), 4);
    ...
    public int[] queryGlobalBestContributor() throws NoUserException {
        ...
        int[] res = new int[2];
        boolean notEmpty = false; // empty or not
        HashMap<Integer, Integer> bestContributors = new HashMap<>(); // K: UID, V: times
        for (User u : users.values()) {
            HashMap<Integer, Integer> contributors = u.getContributors();
            ...
            for (HashMap.Entry<Integer, Integer> entry : contributors.entrySet()) {
                ...
                if (amount > maxAmount) {
                    ...
                    bestId = Math.min(bestId, id);
                }
            }
            bestContributors.put(bestId, bestContributors.getOrDefault(bestId, 0) + 1);
        }
        if (!notEmpty) {
            res[0] = 0;
            res[1] = 0;
        ...
        int maxTimes = -1;
        int bestId = Integer.MAX_VALUE;
        for (HashMap.Entry<Integer, Integer> entry : bestContributors.entrySet()) {
            int id = entry.getKey();
    ...
    public int recommendVideo(int userId)
        ...
        if (getUser(userId).noWatchedVideo()) {
        ...
        for (Video video : videos.values()) {
            ...
            if (score > maxScore) {
                ...
                bestId = Math.min(bestId, video.getId());
            }
        }
        return bestId;
    }
    ...
    public int recommendNthUp(int userId, int rank)
        ...
        HashMap<User, Long> scores = new HashMap<>(); // K: User, V: score
        for (User u : users.values()) {
            if (u.getId() != userId && !user.isFollowing(u)) {
                long score = user.computeUpScore(u, totalVideos);
                candidates.add(u);
                scores.put(u, score);
            }
        }
        ...
        candidates.sort((a, b) -> {
            long scoreA = scores.get(a);
            long scoreB = scores.get(b);
    ...
    public int queryMostInfluentialUp(String type) throws InvalidTypeException, NoUserException {
        ...
        int bestId = Integer.MAX_VALUE;
        for (User u : users.values()) {
            if (u.getInfluence(type) > maxInfluence) {
                maxInfluence = u.getInfluence(type);
                bestId = u.getId();
            }
            else if (u.getInfluence(type) == maxInfluence) {
                bestId = Math.min(bestId, u.getId());
            }
        }
        return bestId;
    }
    ...
    public List<Integer> queryUserProfile(int userId)
        ...
        return getUser(userId).getProfile(videos.size());
    }
